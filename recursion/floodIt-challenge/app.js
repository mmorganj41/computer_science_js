...
function recursiveFlood(x, y, targetValue, newValue) {
    ...
    let adjacentSquares = [new Coordinates(x + 1, y), new Coordinates(x - 1, y), new Coordinates(x, y + 1), new Coordinates(x, y - 1),];

    // change square color and render
    gameBoard[y][x] = newValue;
    renderSquare(x, y);
    // call recursive function on each adjacent square
    for (let adjacentSquare of adjacentSquares) {
        // check if out of bounds
        if (adjacentSquare.y >= gameBoard.length || adjacentSquare.y < 0 || adjacentSquare.x < 0 || adjacentSquare.x >= gameBoard[0].length) continue;

        // check if target color and if so, call recursively
        if (gameBoard[adjacentSquare.y][adjacentSquare.x] === targetValue) recursiveFlood(adjacentSquare.x, adjacentSquare.y, targetValue, newValue);
    }
