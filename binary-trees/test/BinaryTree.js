...
describe('BinaryTree', () => {
    ...
    it('should return the correct balance from a given node', () => {
        const tree = new BinaryTree();
        tree.insert(1);
        expect(tree.isBalanced()).to.equal(true);
        tree.insert(3);
        expect(tree.isBalanced()).to.equal(true);
        expect(tree.isBalanced(tree.root.right)).to.equal(true);
        tree.insert(5);
        expect(tree.isBalanced()).to.equal(false);
        expect(tree.isBalanced(tree.root.right)).to.equal(true);
        expect(tree.isBalanced(tree.root.right.right)).to.equal(true);
    });
})